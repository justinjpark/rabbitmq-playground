...
# Run two instances of worker.py, then in a third console publish the following new tasks:
...
# We also implement 'message acknowledgment' and 'message durability', 
# to ensure messages are re-queued if a consumer dies and that messages are persistent
...
def new_task():
    ...
    # read in command line argument as message
    message = ' '.join(sys.argv[1:]) or 'Hello world!'

    # ensure messages are persistent by setting the delivery_mode property to 2
    properties = pika.BasicProperties(delivery_mode=2)

    # send the message to the queue and close connection
